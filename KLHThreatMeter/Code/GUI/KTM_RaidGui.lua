...
function KLHTM_GetRaidData()
	...
	local target
	local targetnamelevel
	local unit
	
	target = "target"
	if(UnitExists(target)) then	
		targetnamelevel = UnitName(target)..UnitLevel(target)
	end
		
	if(targetnamelevel ~= nill)
	then
		...
		if inRaid then
			for i = 1, 40 do
				...
				if(UnitExists(unit)) then
					playerCount = playerCount + 1;		
					local isTanking, state, scaledPercent, rawPercent, threatValue = ThreatUnitDetailedNameThreatSituation(unit, targetnamelevel)
					if(threatValue == nil) then threatValue = 0 end
		
					if (not(options.raid.hideZeroRows and (threatValue == 0))) then
						rowCount = rowCount + 1;
						data[rowCount].name = UnitName(unit);
						data[rowCount].threat = threatValue;
					end	
				end
