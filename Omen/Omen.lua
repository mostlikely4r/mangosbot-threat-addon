...
local UnitDetailedThreatSituation = ThreatUnitDetailedThreatSituation
...
function Omen:CreateFrames()
	...
	self.ModuleList = CreateFrame("Frame", "OmenModuleButtons", self.Anchor) --pfui compatiblity
...
function Omen:CombatStart(event)
	timers.UpdateBars = self:ScheduleRepeatingTimer("UpdateBars", 0.5)
end

function Omen:CombatStop(event)
...
local lastNonEventUpdate = GetTime()

function Omen:CHAT_MSG_COMBAT_MISC_INFO(...)
	if GetTime() - lastNonEventUpdate > 0.5 then
		self:UpdateBars()
		lastNonEventUpdate = GetTime()
	elseif not timers.UpdateThreatNonEvent then
		timers.UpdateThreatNonEvent = self:ScheduleTimer("CHAT_MSG_COMBAT_MISC_INFO", 0.5)
	end
end
...
local function updatethreat(unitid, mobunitid)
	...
	if guid and not threatTable[guid] then
		local isTanking, state, scaledPercent, rawPercent, threatValue = ThreatUnitDetailedThreatSituation(unitid, mobunitid)
		if threatValue then
...
function Omen:UpdateBarsReal()
	...
	--DevTools_Dump(threatTable)
	--DevTools_Dump(sortTable)
...
local timeSinceLastTPSUpdate = GetTime()

function Omen:UpdateTPS()
	if GetTime() - timeSinceLastTPSUpdate < 0.5 then
		return
	else
		timeSinceLastTPSUpdate = GetTime()
	end
	local numBars = #bars
	if testMode then
	...
	for i = 1, numBars do
		...
		if guid == "AGGRO" then
			...
			if baseThreat and secondThreat and finalThreat then
				...
				bar.Text3:SetFormattedText("%d", (finalThreat - startThreat) / TPSWindow)
			else
